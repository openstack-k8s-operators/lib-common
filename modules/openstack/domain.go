...
func (o *OpenStack) CreateDomain(log logr.Logger, d Domain) (string, error) {
	var domainID string
	allPages, err := domains.List(o.osclient, domains.ListOpts{Name: d.Name}).AllPages(context.TODO())
	if err != nil {
		return domainID, err
	}
	...
	if len(allDomains) == 1 {
		...
		domain, err := domains.Create(context.TODO(), o.osclient, createOpts).Extract()
		if err != nil {
			return domainID, err
		}
		domainID = domain.ID
